...
RSpec.describe 'Basket' do
  ...
  it 'calculates example baskets correctly' do
    b1 = Basket.new(catalogue: catalogue, delivery_rules: delivery_rules, offers: offers)
    b1.add('B01').add('G01')
    expect(b1.total.to_s).to eq('$37.85')

    b2 = Basket.new(catalogue: catalogue, delivery_rules: delivery_rules, offers: offers)
    b2.add('R01').add('R01')
    expect(b2.total.to_s).to eq('$54.38')

    b3 = Basket.new(catalogue: catalogue, delivery_rules: delivery_rules, offers: offers)
    b3.add('R01').add('G01')
    expect(b3.total.to_s).to eq('$60.85')

    b4 = Basket.new(catalogue: catalogue, delivery_rules: delivery_rules, offers: offers)
